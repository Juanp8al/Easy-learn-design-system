...
# CSRF_TRUSTED_ORIGINS="https://mi-dominio.com" (comma-separated) in production.
_env_csrf = [
    o.strip()
...
# En desarrollo suele fallar el 403 si `runserver` usa otro puerto o se entra por la IP LAN:
# el navegador envía Origin/Referer con ese host:puerto y debe estar en la lista.
if DEBUG or DEVELOPMENT_MODE or LOCAL_SERVER:
    _dev_ports = [
        p.strip()
        for p in os.getenv("DJANGO_DEV_SERVER_PORTS", "8000,8001,8080,8888").split(",")
        if p.strip()
    ]
    ...
    for host in ALLOWED_HOSTS:
        if host.startswith("."):
            continue
        _dev_hosts.add(host)
    for host in sorted(_dev_hosts):
        for port in _dev_ports:
            origin = f"http://{host}:{port}"
            if origin not in CSRF_TRUSTED_ORIGINS:
                CSRF_TRUSTED_ORIGINS.append(origin)
    # Sin HTTPS local, las cookies "Secure" impiden enviar csrftoken/session y Django rechaza el POST.
